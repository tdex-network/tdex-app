...
export class ElectrumWS extends Observable {
  ...
  constructor(endpoint: string, options: Partial<ElectrumWSOptions> = {}) {
    ...
    Object.values(ElectrumWSEvent).forEach((ev: string) => {
      this.on(ev, (e: any) =>
        e ? console.debug(`ElectrumWS - ${ev.toUpperCase()}:`, e) : console.debug(`ElectrumWS - ${ev.toUpperCase()}`)
      );
    });
  }

  public async batchRequest<R extends any[]>(...requests: { method: string; params: any[] }[]): Promise<R> {
    if (!this.connected) {
  ...
  public async unsubscribe(method: string, ...params: (string | number)[]): Promise<void> {
    const subscriptionKey = `${method}${typeof params[0] === 'string' ? `-${params[0]}` : ''}`;
